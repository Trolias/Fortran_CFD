...
! dimensions:
      integer :: iNxCells, iNyCells, iNxNodes, iNyNodes, iNxUNodes, iNyUnodes, & 
                 iNxVNodes,iNyVNodes,iNxPNodes,iNyPNodes, iNxMax, iNyMax, Schemee
!iterations
...
! read parameters from file
      ...
      open (unit=1, file="pars.par",status="old")
      read (1,*) rHeight
      read (1,*) rWidth
      read (1,*) rReynolds
      read (1,*) rPeclet
...
! scalar - same control volume as P
      allocate(rU(iNxMax,iNyMax),rV(iNxMax,iNyMax),rP(iNxMax,iNyMax))
...
! compute u-momentum equation
       call UMomentum(iNxUNodes, iNyUNodes,iNxMax,iNyMax,rDx,rDy,rU,rV,rP,rDU,rAlphaU,rURes,rReynolds,Schemee)
! compute v-momentum equation
       call VMomentum(iNxVNodes, iNyVNodes,iNxMax,iNyMax,rDx,rDy,rU,rV,rP,rDV,rAlphaV,rVRes,rReynolds,Schemee)
! correct mass flux
       rTotFlow=0.
       do j = 2 , iNyUNodes-1
...
!output tecplot file:
      open (unit=1,file="FINAL_REYNOLDS.dat",status="replace",action="write")
      write(1,*) 'Variables="X","Y","P","U","V"'
      ...
      !    rCCur=0.25*(rC(i,j)+rC(i+1,j)+rC(i,j+1)+rC(i+1,j+1))
         ...
         write(1,"(8E15.7)") (i-1)*rDX,(j-1)*rDY,rPCur,rUCur,rVCur
       end do
      end do
      close(1)
...
subroutine UMomentum(iNxUNodes, iNyUNodes,iNxMax,iNyMax,rDx,rDy,rU,rV,rP,rDU,rAlphaU,rURes,rReynolds,Schemee)
!arguments
    integer, intent(in) :: iNxUNodes, iNyUNodes,iNxMax,iNyMax,Schemee   
...
! north
    ...
    an(:,iNyUNodes)=0.
    ap(:,iNyUNodes)=1. 
    su(:,iNyUNodes)=1. ! equals zero for v-velocity
! east
    aw(iNxUNodes,:)=0.
    ae(iNxUNodes,:)=0.
    as(iNxUNodes,:)=0.
      ...
      do i = 2 , iNxUNodes-1
         do J = 2 , iNyUNodes-1
            ...
            CASE(2)
                  ...
                  as(i, j) = ds + 0.5 * gs
                  an(i, j) = dn - 0.5 * gn
                  ap(i, j) = dw + de + ds + dn + 0.5*(ge-gw+gn-gs)

            CASE DEFAULT
                  print*,' Wrong numerical scheme selection.'
                  stop
            END SELECT
...
subroutine VMomentum(iNxVNodes, iNyVNodes,iNxMax,iNyMax,rDx,rDy,rU,rV,rP,rDV,rAlphaV,rVRes,rReynolds,Schemee)
!arguments
    integer, intent(in) :: iNxVNodes, iNyVNodes,iNxMax,iNyMax,Schemee
